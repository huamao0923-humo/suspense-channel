...
console.log(TYPE_SFX ? '打字機音效：品牌檔' : '打字機音效：合成緩慢有力敲擊聲（放 brand/assets/sfx/typewriter.mp3 可覆寫）');
if (EP.intro && (EP.intro.dateLines || []).length) {
  ...
  console.log(`開場：黑屏打字機日期卡（${EP.intro.dateLines.join(' / ')}，${d.toFixed(1)}s，插在解說員開場後）${TYPE_SFX ? '＋打字機音效' : '＋合成打字機敲擊聲'}`);
}
if (EP.ending) {
...
// sig 同時納入「段落 ID」與「台詞內容雜湊」：只要任一段文字被編輯，sig 即變、整案語音重生（最簡單、零誤接風險）。
{
  ...
  const sigFile = join(AUDIO, '_sig.txt'), sig = segs.map(s => s.id + ':' + djb2(s.narration || s.heading || '')).join(',');
...
// 一般鍵盤敲擊感（非厚重「咚」聲／非鞭炮）：每擊衰減極快（exp(-40)）＝短促清脆「答」聲；
// highpass=200 去低頻轟鳴、lowpass=6000 保鍵盤清脆、volume=0.3 降到背景襯底音量。每 0.42s 一擊。
const synthTypewriter = (wav, d) => sh('ffmpeg', ['-y', '-f', 'lavfi',
  '-i', `aevalsrc=exprs='(random(0)*2-1)*exp(-40*mod(t,0.42))':s=44100:d=${d}`,
  '-af', 'highpass=f=200,lowpass=f=6000,volume=0.3', '-ac', '2', wav]);
for (let i = 0; i < segs.length; i++) {
  if (!segs[i]._synthetic) continue;
  const wav = join(AUDIO, `seg${i}.wav`), d = segs[i]._dur.toFixed(2);
  if (segs[i].kind === 'intro') {
...
function visionPickStart(id, url, matchText) {
  ...
  try {
    mkdirSync(adir, { recursive: true });
    for (const f of readdirSync(adir)) { try { unlinkSync(join(adir, f)); } catch { } }
    execFileSync('py', ['-m', 'yt_dlp', '-q', '--no-warnings', '--no-playlist', '--download-sections', '*0-240',
...
async function youtubeCC(terms, out, matchText) {
  ...
  for (const term of terms) {
    ...
    for (const it of (j.items || [])) {
      ...
      try {
        execFileSync('py', ['-m', 'yt_dlp', '-q', '--no-warnings', '--no-playlist',
...
function manualFor(n, k) {
  ...
  for (const ext of ['mp4', 'webm', 'mov', 'png', 'jpg', 'jpeg', 'webp']) {
    ...
    if (existsSync(f) && statSync(f).size > 2000) return { path: f, video: /mp4|webm|mov/.test(ext) };
  }
  return null;
}
...
// 同一素材最多用 3 次（使用者定調 2026-06-02）：超過就跳過、改抽別張或退 AI 示意（避免整片重複同幾張）。
const usageCount = new Map();
const overUsed = p => (usageCount.get(p) || 0) >= 3;
...
async function acquireMode(mode, n, k, q, prompt, heading) {
  ...
  if (mode === 'video') {
    ...
    // 再來（混合策略）：Pexels，再 Commons 影片（即時爬該段查詢詞）
    const pv = await pexelsVideo(q, vidOut);
    if (pv) { credits.push({ seg: n, k, ...pv, query: q }); console.log(`  s${n}_${k} 〔影片〕Pexels ✓`); return { path: vidOut, video: true }; }
  ...
  if (mode === 'real-photo') {
    ...
    if (cc) { credits.push({ seg: n, k, ...cc, query: q }); console.log(`  s${n}_${k} 〔真實圖〕CC ✓`); return { path: imgOut, video: false }; }
    // ★Pexels 場景圖（優先序第4級，AI 之前）：真實圖取不到才用
    let px = await pexelsImage(q, imgOut); if (!px) for (const g of GENERIC) { px = await pexelsImage(g, imgOut); if (px) break; }
    if (px) { credits.push({ seg: n, k, ...px, query: q }); console.log(`  s${n}_${k} 〔Pexels場景圖〕✓`); return { path: imgOut, video: false }; }
    console.log(`  s${n}_${k} 〔真實圖〕無 → 退 AI 示意`); return acquireMode('illust', n, k, q, prompt, heading);
  }
...
async function mapZoomClip(outPath, place, cd) {
  ...
  if (process.env.PILOT_MAP_SINGLE !== '0') {
    const FR = FPS;
    const Z = Number(process.env.PILOT_MAP_ZOOM || '10');     // 框住城市＋周邊；可用 env 調框
    const zoomDur = Math.min(cd * 0.5, 8);                    // 加快 ~30%（原 cd*0.72/上限 11s）
    ...
    if (existsSync(f) && statSync(f).size > 5000) {
      ...
      //   放大倍率由 0.5→1.0（起點整張入鏡、終點中央 2×），全程 lanczos 重採樣＝平滑。
      //   p = min(t/zoomDur, 1)：前 zoomDur 推近，其餘 hold。源 3840×2160；scale 寬 1920→3840。
      const p = `min(t/${zoomDur.toFixed(3)}\\,1)`;
      let vf = `scale=3840:2160:force_original_aspect_ratio=increase,crop=3840:2160,`;
      vf += `scale=w='3840*(0.5+0.5*${p})':h='2160*(0.5+0.5*${p})':eval=frame:flags=lanczos,crop=1920:1080`;
...
function endingFootage() {
  ...
  if (libVids.length) return { path: libVids[0], video: true };
  if (libImgs.length) return { path: libImgs[0], video: false };
  return null;
}
...
for (let s = 0; s < segs.length; s++) {
  ...
  if (seg.kind === 'intro') { segImages[s].push({ intro: true }); imgCount++; continue; }
  ...
  for (let k = 0; k < prompts.length; k++) {
    ...
    if (man) { segImages[s].push({ path: man.path, video: man.video, illustrative: !man.video }); imgCount++; console.log(`  s${n}_${k} 手動貼圖 ✓`); continue; }
...
for (let s = 0; s < segs.length; s++) {
  ...
  for (let j = 0; j < slots; j++) {
    ...
    let item;
    if (segs[s].kind !== 'host' && j >= imgs.length) {
      let cand = null;
      if (libPool.length) {
        const hasVid = libPool.some(p => p.video);
        let tries = 0; do { cand = libPool[libIdx % libPool.length]; libIdx++; } while (((hasVid && !cand.video) || overUsed(cand.path)) && ++tries < libPool.length * 2);
        if (overUsed(cand.path)) cand = null;
      }
